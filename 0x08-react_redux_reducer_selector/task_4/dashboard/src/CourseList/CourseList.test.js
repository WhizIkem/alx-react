...
describe("CourseList component tests", () => {
  ...
  it("renders 5 different rows", () => {
    ...
    expect(wrapper.find("tbody").childAt(0).find("td").at(0).find("input[type='checkbox']")).toHaveLength(1);
    expect(wrapper.find("tbody").childAt(0).find("td").at(0).text()).toContain("ES6");
    expect(wrapper.find("tbody").childAt(0).find("td").at(1).text()).toBe("60");
  });
