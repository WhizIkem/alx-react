...
test('Clicking on the first button should call setNotificationFilter with URGENT', () => {
  ...
  const { getByTestId } = render(
    <Notifications listNotifications={[]} setNotificationFilter={setNotificationFilter} />
    );
    const urgentButton = getByTestId('urgent-btn');
...
test('Clicking on the second button should call setNotificationFilter with DEFAULT', () => {
  ...
  const { getByTestId } = render(
    ...
    const defaultButton = getByTestId('default-btn');
    fireEvent.click(defaultButton);
    expect(setNotificationFilter).toHaveBeenCalledWith('DEFAULT');
  });

test('Notifications component renders correctly', () => {
  const { getByText, getByTestId } = render(
    <Notifications displayDrawer={true} listNotifications={[]} setNotificationFilter={() => {}} />
    );
});